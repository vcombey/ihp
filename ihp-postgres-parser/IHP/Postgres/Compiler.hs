...
compileConstraint CompositeForeignKeyConstraint { columnNames, referenceTable, referenceColumns, onDelete, onUpdate } = "FOREIGN KEY (" <> intercalate ", " (map compileIdentifier columnNames) <> ") REFERENCES " <> compileQualifiedIdentifier referenceTable <> (if null referenceColumns then "" else " (" <> intercalate ", " (map compileIdentifier referenceColumns) <> ")") <> compileOnUpdate onUpdate <> " " <> compileOnDelete onDelete
...
-- | The shortest dollar quote delimiter that does not occur in the body.
--
-- PostgreSQL does the same: @$$@ unless the body contains it, then @$_$@, and so
-- on. Choosing it here rather than carrying the original delimiter keeps the
-- function representation to what the schema needs while staying lossless.
